 ...
 */
public class DepartSixPage {
/**
 * 	生成部门或者课别第六页（部门集记）的集记信息
 ...
 * @param listProjectTasks   所有的作业类型的集合
 * @param workDayslist   每个月上班天数的集合
 */
	public void updateSixXLS(String department,String branch,List<Project> listProject,int year, int month,
		...
		try {
			...
			String strnameString1=Parma.TEMP_FILESixthAddress+"/"+"集計データ"+department+year+"年"+month+"月"+".xls";//生成的模板存储所在地
			...
			//add a Sheet.
			WritableSheet sheetSix;
			if (branch==null) {
				sheetSix = book1.createSheet(department+"集计", 5);
			}
			else {
				sheetSix = book1.createSheet(department+branch+"集计", 5);
			}
			...
			Label labelaLabel = new Label(1, 0, "集记日",formatgr);
			...
			if (branch==null) {
				...
				Label labelbLabelfg = new Label(2, 1, department+branch,formatgr);
				sheetSix.addCell(labelbLabelfg);
			}
...
//					Label labelab1ab = new Label(2, rownum, listFirsttaskname.get(i),formatleft);
				for (int i = 0; i < listFirsttask.size(); i++) {
					Label labelab1ab = new Label(2, rownum, listFirsttask.get(i).getTask(),formatleft);
			...
			for (int m = 0; m < Parma.strProjectClientNameDevelopPage.length; m++) {
				...
				for (int i = 0; i < listProject.size(); i++) {
					...
					for (int i = 0; i < listSecondtask.size(); i++) {
						Label labelab1ab = new Label(2, rownum, listSecondtask.get(i).getTask(),formatleft);
						sheetSix.addCell(labelab1ab);
						double sumtimetemp=0.0;
						int weiyitemp=weiyi;
						for (int s = 0; s < allMonthListManHourDtoList.size(); s++) {
			...
			for (int m = 0; m < Parma.strProjectClientNameDevelopPage.length; m++) {
				...
				for (int i = 0; i < listProject.size(); i++) {
					...
					for (int i = 0; i < listThirdtask.size(); i++) {
						Label labelab1ab = new Label(2, rownum, listThirdtask.get(i).getTask(),formatleft);
						sheetSix.addCell(labelab1ab);
						double sumtimetemp=0.0;
						int weiyitemp=weiyi;
						for (int s = 0; s < allMonthListManHourDtoList.size(); s++) {
			...
			if (listFourthtask.size()!=0) {
				...
				rownum++;
						...
						for (int i = 0; i < listFourthtask.size(); i++) {
							Label labelab1ab = new Label(2, rownum, listFourthtask.get(i).getTask(),formatleft);
							sheetSix.addCell(labelab1ab);
							double sumtimetemp=0.0;
							int weiyitemp=weiyi;
