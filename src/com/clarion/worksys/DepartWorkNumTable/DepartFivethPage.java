 ...
 */
public class DepartFivethPage {
/**
 * 	生成部门或者课别第五页（开发工数）的信息
 ...
 */
	public void updateFivethXLS(String department,String branch,List<Project> listProject,int year, int month,
		...
		try {
			...
			String strnameString1=Parma.TEMP_FILEFiveAddress+"/"+"集計データ"+department+year+"年"+month+"月"+".xls";//生成的模板存储所在地
			String strnameString2=Parma.TEMP_FILESixthAddress+"/"+"集計データ"+department+year+"年"+month+"月"+".xls";//由模板生成的最终文件所在地址
			...
			//add a Sheet.
			WritableSheet sheetFive;
			if (branch==null) {
				sheetFive = book1.createSheet(department, 4);
			}
			else {
				sheetFive = book1.createSheet(department+branch, 4);
			}
			...
			formatgr.setAlignment(jxl.format.Alignment.LEFT); 
			...
			formatleft.setVerticalAlignment(jxl.format.VerticalAlignment.CENTRE); //垂直居中
			...
			if (branch==null) {
				...
				Label labelbLabelfg = new Label(3, 1, department+branch,formatgr);
				sheetFive.addCell(labelbLabelfg);
			}
...
//			List<String> listFourthtaskname=new ArrayList<String>();
			List<Project_task> listFirsttask=new ArrayList<Project_task>();
			List<Project_task> listSecondtask=new ArrayList<Project_task>();
			List<Project_task> listThirdtask=new ArrayList<Project_task>();
			List<Project_task> listFourthtask=new ArrayList<Project_task>();
			...
			sheetFive.mergeCells(0,rownum,2,rownum+1);
			Label labela1 = new Label(0, rownum, "総工数",formattitleor);
			...
			for (int m = 0; m < Parma.strProjectClientNameDevelopPage.length; m++) {
				...
				for (int k = 0; k < listProjectCT.size(); k++) {
					...
					//}else if (("工程设计部").equals(department)&&("185 工程设计部DCOE").equals(listProjectCT.get(k).getProjectName())) {
						...
						//Label labelaea2 = new Label(2, rownum-Parma.strAdditionalProjDesignerDCOE.length, listProjectCT.get(k).getProjectName(),formatgr);
						//sheetFive.addCell(labelaea2);
					...
					double[] sumtempTol=new double[allMonthListManHourDtoList.size()];
						...
						for (int i = 0; i < listFirsttask.size(); i++) {
							Label labelab1ab = new Label(3, rownum, listFirsttask.get(i).getTask(),formatleft);
							sheetFive.addCell(labelab1ab);
							double sumtimetemp=0.0;
							int weiyitemp=weiyi;
								...
								for (int j = 0; j < allMonthListManHourDtoList.get(s).size(); j++) {
									if (("开发工数").equals(allMonthListManHourDtoList.get(s).get(j).getCategory())) {
										if (listProjectCT.get(k).getProjectName().equals(allMonthListManHourDtoList.get(s).get(j).getProjectName())) {
											...
											if (listFirsttask.get(i).getTaskID().equals(allMonthListManHourDtoList.get(s).get(j).getTaskID())) {
...
//						sheetFive.mergeCells(0,rownum-listFirsttaskname.size(),0,rownum);
...
//						Label labelab1ab = new Label(3, rownum, listSecondtaskname.get(i),formatleft);
					for (int i = 0; i < listSecondtask.size(); i++) {
						Label labelab1ab = new Label(3, rownum, listSecondtask.get(i).getTask(),formatleft);
						sheetFive.addCell(labelab1ab);
						double sumtimetemp=0.0;
						int weiyitemp=weiyi;
							...
							for (int j = 0; j < allMonthListManHourDtoList.get(s).size(); j++) {
								if (("RFI和RFQ").equals(allMonthListManHourDtoList.get(s).get(j).getCategory())) {
									if (listRFIRProjectCT.get(k).getProjectName().equals(allMonthListManHourDtoList.get(s).get(j).getProjectName())) {
										...
										if (listSecondtask.get(i).getTaskID().equals(allMonthListManHourDtoList.get(s).get(j).getTaskID())) {
											sumtime = Arith.add(sumtime,allMonthListManHourDtoList.get(s).get(j).getTimes());
										}
									}
								}
					...
					//Label labelaea2 = new Label(2, rownum-listSecondtaskname.size(), listRFIRProjectCT.get(k).getProjectName(),formatgr);
					//sheetFive.addCell(labelaea2);
...
//					sheetFive.mergeCells(0,rownum-listSecondtaskname.size(),0,rownum);
...
//						Label labelab1ab = new Label(3, rownum, listThirdtaskname.get(i),formatleft);
					for (int i = 0; i < listThirdtask.size(); i++) {
						Label labelab1ab = new Label(3, rownum, listThirdtask.get(i).getTask(),formatleft);
						sheetFive.addCell(labelab1ab);
						double sumtimetemp=0.0;
						int weiyitemp=weiyi;
							...
							for (int j = 0; j < allMonthListManHourDtoList.get(s).size(); j++) {
								if (("量产后不具合对应").equals(allMonthListManHourDtoList.get(s).get(j).getCategory())) {
									if (listRFIRProjectCT.get(k).getProjectName().equals(allMonthListManHourDtoList.get(s).get(j).getProjectName())) {
										...
										if (listThirdtask.get(i).getTaskID().equals(allMonthListManHourDtoList.get(s).get(j).getTaskID())) {
											sumtime = Arith.add(sumtime,allMonthListManHourDtoList.get(s).get(j).getTimes());
										}
									}
								}
					...
					//Label labelaea2 = new Label(2, rownum-listThirdtaskname.size(), listRFIRProjectCT.get(k).getProjectName(),formatgr);
					//sheetFive.addCell(labelaea2);
...
//							if(k1 == (rownum-listThirdtaskname.size())){
					...
					//sheetFive.mergeCells(0,rownum-listThirdtaskname.size(),0,rownum);
			...
			if (listFourthtask.size()!=0) {
				...
				Label labelab12ng = new Label(3, rownum, "Tol",formattitleor);
				...
				Number labeladgn=new Number(28, rownum,categorysumtime4,formatNum);
				sheetFive.addCell(labeladgn);
				Number labelbrt=new Number(29, rownum,Arith.div(categorysumtime4,personMonthTotal),formatNum);
				sheetFive.addCell(labelbrt);
				Number labeldty=new Number(30, rownum,Arith.div(categorysumtime4, basetotalnum, 4)*100,formatNum);
				sheetFive.addCell(labeldty);
				rownum++;
						...
						for (int i = 0; i < listFourthtask.size(); i++) {
							//Label labelab1ab = new Label(3, rownum, listFourthtaskname.get(i),formatleft);
							Label labelab1ab = new Label(3, rownum, listFourthtask.get(i).getTask(),formatleft);
							sheetFive.addCell(labelab1ab);
							double sumtimetemp=0.0;
							int weiyitemp=weiyi;
							...
							for (int s = 0; s < allMonthListManHourDtoList.size(); s++) {
								...
								for (int j = 0; j < allMonthListManHourDtoList.get(s).size(); j++) {
									if (("先行开发").equals(allMonthListManHourDtoList.get(s).get(j).getCategory())) {
										...
										if (listFourthtask.get(i).getTaskID().equals(allMonthListManHourDtoList.get(s).get(j).getTaskID())) {
												sumtime = Arith.add(sumtime,allMonthListManHourDtoList.get(s).get(j).getTimes());
							...
							Number labeladb23=new Number(28, rownum,sumtimetemp,formatright);
							sheetFive.addCell(labeladb23);
							Number labelcju=new Number(29, rownum,Arith.div(sumtimetemp,personMonthTotal),formatright);
							sheetFive.addCell(labelcju);
							Number labeletg=new Number(30, rownum,Arith.div(sumtimetemp, basetotalnum, 4)*100,formatright);
						...
						Number labelada =new Number(28, rownum,sumtotalCT,formatNum);
						sheetFive.addCell(labelada);
						Number labelcju=new Number(29, rownum,Arith.div(sumtotalCT,personMonthTotal),formatNum);
						sheetFive.addCell(labelcju);
						Number labeletg=new Number(30, rownum,Arith.div(sumtotalCT, basetotalnum, 4)*100,formatNum);
						sheetFive.addCell(labeletg);
//						sheetFive.mergeCells(2,rownum-listFourthtaskname.size(),2,rownum);
//						Label labelaea2 = new Label(2, rownum-listFourthtaskname.size(), department,formatleft);
						sheetFive.mergeCells(2,rownum-listFourthtask.size(),2,rownum);
						Label labelaea2 = new Label(2, rownum-listFourthtask.size(), department,formatleft);
						sheetFive.addCell(labelaea2);
						rownum++;
			}
			/**
