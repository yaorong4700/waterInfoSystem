 ...
 * @param allMonthListManHourDtoList   所有的工数数据信息
 * @param listProject    所有项目的集合
 * @param listProjectTasks    所有的作业类型的集合
 * @param allListCategoryDetail   category详细的工数信息
 * @param workDayslist    每个月上班天数的集合
 * @param allAssumePJDetailList   关于PJ担当的详细信息集合
 * @param designStaffNum   设计要员数
 */
	public void departFirstPage(String department,String branch, int year,int month,int endyear, int endmonth, String filenameString,
		...
		try {
		    ...
		    Label label0_0 = new Label(0, 0, "集记数据",formato);
			Label label0_1 = new Label(1, 0, "集记日",formato);
			...
			for (int i = 0; i < workDayslist.size(); i++) {
				...
				Number labeldef=new Number(weiyiTime,rowNumTime+2,designStaffNum.get(i),formatoNum);
				sheet.addCell(labeldef);
				Number labelhyt=new Number(weiyiTime,rowNumTime+3,designStaffNum.get(i)*workDayslist.get(i)*8,formatoNum);
				...
				for (int i = 0; i < allMonthListManHourDtoList.get(m).size(); i++) {
					for (int j2 = 0; j2 < listProjectTasks.size(); j2++) {
						if ((Parma.strDevelopWorkNum[0]).equals(listProjectTasks.get(j2).getMemo())) {
							...
							if (listProjectTasks.get(j2).getTaskID().equals(allMonthListManHourDtoList.get(m).get(i).getTaskID())) {
...
//			}
			for (int i = 0; i < Parma.strDevelopWorkNum.length; i++) {
				...
				for (int j = 0; j < allMonthListManHourDtoList.size(); j++) {
							...
							if (allMonthListManHourDtoList.get(j).get(m).getProjectID()==(listProjectCT.get(j2).getProjectID())) {
									for (int k2 = 0; k2 < listProjectTasks.size(); k2++) {
										if ((Parma.strDevelopWorkNum[i]).equals(listProjectTasks.get(k2).getMemo())) {
											...
											if (listProjectTasks.get(k2).getTaskID().equals(allMonthListManHourDtoList.get(j).get(m).getTaskID())) {
					...
					sumtemp+=sum91;
					Number label2=new Number(weiyi9, rowStartnum,sum91+listsum9.get(j),formatoNum);
					sheet.addCell(label2);
					Number labeltu=new Number(weiyi9+1, rowStartnum,Arith.div(sum91+listsum9.get(j),workDayslist.get(j)*8),formatoNum);
					sheet.addCell(labeltu);
			...
			for (int i = 0; i < Parma.strDevelopWorkNum.length; i++) {
				...
				for (int j = 0; j < allMonthListManHourDtoList.size(); j++) {
					...
					sumtemp+=sum91;
					Number label2=new Number(weiyi9, rowStartnum,sum91+listsum10.get(j),formatoNum);
					sheet.addCell(label2);
					Number labeltu=new Number(weiyi9+1, rowStartnum,Arith.div(sum91+listsum10.get(j),workDayslist.get(j)*8),formatoNum);
					sheet.addCell(labeltu);
