 ...
 */
public class DepartThirdPage_ForCt {
/**
 * 生成部门/课别第三页(RFI&RFQ)信息_ForCt
 * @param user   出力者
 ...
 */
	public void updateThirdXLS_ForCt(String user, String department,String branch,List<Project> listProject,int year, int month,
			int endyear, int endmonth, String filenameString,List<List<ManHourDto>> allMonthListManHourDtoList,List<Project_task> listProjectTasks
		...
		try {
			...
			WritableSheet sheetTwo = book1.createSheet(Parma.strTotalWorkNumName_ForCt[1], 2);
			...
			Label label0_1 = new Label(9, 1, Parma.strTotalWorkNumName_ForCt[1],formato_title1);
			...
			for (int i = 0; i < listProjectTasks.size(); i++) {
				if (Parma.strTotalWorkNumName_ForCt[1].equals(listProjectTasks.get(i).getCategory())) {
			...
			for (int i = 0; i < listDepartSencouTasks.size(); i++) {
				...
				for (int m = 0;m < allMonthListManHourDtoList.size();m++) {
					...
					for (int j = 0; j < allMonthListManHourDtoList.get(m).size(); j++) {
						if (Parma.strTotalWorkNumName_ForCt[1].equals(allMonthListManHourDtoList.get(m).get(j).getCategory())) {
			...
			for (int i = 0; i < allMonthListManHourDtoList.size(); i++) {
				for (int j = 0; j < allMonthListManHourDtoList.get(i).size(); j++) {
					if (Parma.strTotalWorkNumName_ForCt[1].equals(allMonthListManHourDtoList.get(i).get(j).getCategory())) {
			...
			for (int i = 0; i < listProject.size(); i++) {
				if (Parma.strTotalWorkNumName_ForCt[1].equals(listProject.get(i).getCategory())) {
