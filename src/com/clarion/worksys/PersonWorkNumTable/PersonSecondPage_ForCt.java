 ...
 */
public class PersonSecondPage_ForCt {
/**
 * 生成个人集计第二页（先行開発）信息_ForCt
 * @param user   出力者
 ...
 */
	public void updatePersonWorkNumSecondXLS_ForCt(String user, String department,String branch,List<Project> listProject,int year, int month,
		...
		try {
			...
			String strnameString1=Parma.TEMP_FILESecondAddress+"/"+"集計データ"+department+year+"年"+month+"月"+uniqueString+".xls";//生成的模板存储所在地
			String strnameString2=Parma.TEMP_FILEThirdAddress+"/"+"集計データ"+department+year+"年"+month+"月"+uniqueString+".xls";//由模板生成的最终文件所在地址
			...
			//add a Sheet.
			WritableSheet sheetTwo = book1.createSheet(Parma.strTotalWorkNumName_ForCt[0], 1);
			...
			Label label0_1 = new Label(9, 1, Parma.strTotalWorkNumName_ForCt[0],formato_title1);
			...
			for (int i = 0; i < listProjectTasks.size(); i++) {
				if (Parma.strTotalWorkNumName_ForCt[0].equals(listProjectTasks.get(i).getCategory())) {
			...
			for (int i = 0; i < listDepartSencouTasks.size(); i++) {
				...
				for (int m = 0;m < allMonthListManHourDtoList.size();m++) {
					...
					for (int j = 0; j < allMonthListManHourDtoList.get(m).size(); j++) {
						if (Parma.strTotalWorkNumName_ForCt[0].equals(allMonthListManHourDtoList.get(m).get(j).getCategory())) {
			...
			for (int i = 0; i < allMonthListManHourDtoList.size(); i++) {
				for (int j = 0; j < allMonthListManHourDtoList.get(i).size(); j++) {
					if (Parma.strTotalWorkNumName_ForCt[0].equals(allMonthListManHourDtoList.get(i).get(j).getCategory())) {
			...
			for (int i = 0; i < listProject.size(); i++) {
				if (Parma.strTotalWorkNumName_ForCt[0].equals(listProject.get(i).getCategory())) {
