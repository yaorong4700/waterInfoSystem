 ...
 */
public class CompanySecondPage {
/**
 * 生成公司整体集记第二页（量产后不具合对应）的数据
 * @param department   此处为“公司整体”
 * @param listProject   现有的项目集合
 * @param year   起始年份
 * @param month   起始月份
 * @param filenameString   唯一性字符串，确保文件不会重名
 * @param allMonthListManHourDtoList   所有的工数信息的集合
 * @param listProjectTaskstemp   所有的作业类型的集合
 * @param workDayslist     每个月上班天数的集合
 */
	public void updateSecondXLS(String department,List<Project> listProject,int year, int month,
			                     String filenameString,List<List<ManHourDto>> allMonthListManHourDtoList,List<Project_task> listProjectTaskstemp
		...
		try {
			...
			WritableCellFormat formatNum=new WritableCellFormat(fonttitle); //data alignment left set
			...
			WritableCellFormat formatTitle=new WritableCellFormat(fonttitle); //data alignment left set
			...
			WritableCellFormat formatbl=new WritableCellFormat(fonttitle); 
			...
			for (int j = 0; j < listProjectTaskstemp.size(); j++) {
					...
					for (int i = j+1; i < listProjectTaskstemp.size(); i++) {
						if (temp.equals(listProjectTaskstemp.get(i).getTask())) {
							if (listProjectTaskstemp.get(j).getCategory().equals("量产后不具合对应")) {
								if (listProjectTaskstemp.get(i).getCategory().equals("量产后不具合对应")) {
			...
			for (int i = 0; i < listProjectTaskstemp.size(); i++) {
				if (("量产后不具合对应").equals(listProjectTaskstemp.get(i).getCategory())) {
					if (listProjectTaskstemp.get(i).getTask().isEmpty()) {
						
					}
					else {
						listDepartBuJuHeTasks.add(listProjectTaskstemp.get(i));
					}
				}
			}
			...
			for (int i = 0; i < allMonthListManHourDtoList.size(); i++) {
				for (int m= 0; m < allMonthListManHourDtoList.get(i).size(); m++) {
					if (("量产后不具合对应").equals(allMonthListManHourDtoList.get(i).get(m).getCategory())) {
						sumTotalBuJuHe = Arith.add(sumTotalBuJuHe,allMonthListManHourDtoList.get(i).get(m).getTimes());
					}
				}
			}
			for (int i = 0; i < listDepartBuJuHeTasks.size(); i++) {
				...
				Number labelef=new Number(28, rownum,Arith.div(sumTasktemp, sumTotalBuJuHe, 4)*100,formatNum);
			...
			for (int i = 0; i < listDepartBuJuHeProjects.size(); i++) {
				...
				for (int m = 0; m < allMonthListManHourDtoList.size(); m++) {
					...
					for (int j = 0; j < allMonthListManHourDtoList.get(m).size(); j++) {
						if (("量产后不具合对应").equals(allMonthListManHourDtoList.get(m).get(j).getCategory())) {
							if (listDepartBuJuHeProjects.get(i).getProjectID()==(allMonthListManHourDtoList.get(m).get(j).getProjectID())) {
								sumTask = Arith.add(sumTask,allMonthListManHourDtoList.get(m).get(j).getTimes());
							}
						}
					}
			...
			* 对剩余的空白部分进行单元格处理
			*/
			WritableCellFormat formatg=new WritableCellFormat(fonttitle);
