...
.loading-indicator-bars {
	...
	function setPageByLanguage(){
		...
		E0098=$.i18n.prop('E0098');
		E0099=$.i18n.prop('E0099');
		...
		$('#customDemo_string_alterComponentID').html($.i18n.prop('customDemo_string_component'));
		$('#customDemo_string_componentID').html($.i18n.prop('customDemo_string_component'));
...
$(document).ready(function(){	
    ...
    $("#alter_componentID").select2({
        width:420
     });
    $("#alter_taskName").select2({
        width:420
     });
	var clickDate = "";
	var clickAgendaItem = "";
	...
	function myApplyTooltip(divElm,agendaItem){
		...
		for (var propertyName in data) {
			if(propertyName == "manhourID" || propertyName == "ProjectID" || propertyName == "TaskID" || propertyName == "ComponentID"){
...
//	jfcalplugin.setAspectRatio("#mycal",1);
	...
	function myDayClickHandler(eventObj){
		...
		var monthEndFlag = $("#monthEndFlag").val();
	...
	*******************************/
	 ...
	 $("#alter-event-form").dialog({
			...
			buttons: {
				'Save': function() {
					...
					if($("#alter_projectID").val() == null || $("#alter_projectID").val() == ""){
						...
						if ($("#alter_componentID").val() == null || $("#alter_componentID").val() == ""){
							hideloading();
							alert(E0098);
						} else {
							if (what2 ==""||what2<=0||what>24) {//新增
								...
								var myComponentId = $("#alter_componentID").val();
								//20140904数据合法性检查
								$.ajax({ 
			                        type: "post", 
			...
			open: function(event, ui){
				...
				$("#AlterEventCurrentDay").html(day);
					...
					var componentName  = data.コンポーネント;
					...
					projectSel.onchange(); 
					var componentSel = document.getElementById("alter_componentID");
					componentSel.onchange();
					var altertaskNameSel = document.getElementById("alter_taskName");
					altertaskNameSel.onchange(); 
					
					$("#alter_projectID").attr("value",ProjectID).trigger("change");;
					$("#alter_componentID").attr("value",ComponentID).trigger("change");
					$("#alter_taskName").attr("value",TaskID).trigger("change");;
	...
	$("#add-event-form").dialog({
		...
		buttons: {
			'Add': function() {
				...
				if($("#categoryName").val() == null || $("#categoryName").val() == ""){
					hideloading();
					alert(E0099);
				}else if($("#projectID").val() == null || $("#projectID").val() == ""){
					hideloading();
					alert(E0099);
				} else {
					if ($("#componentID").val() == null || $("#componentID").val() == ""){
						hideloading();
						alert(E0099);
					}else if($("#taskName").val() == null || $("#taskName").val() == ""){
						hideloading();
						alert(E0099);
					} else if(what2 ==""){
						hideloading();
						...
						if (what2 ==""||what2<=0||what>24) {//新增
							...
							var myComponentID = $("#componentID").val();
							//20140904数据合法性检查
							$.ajax({ 
		                        type: "post", 
		...
		open: function(event, ui){		
			...
			$("#ProjectCategory").focus();
			//$("#taskName").val($("#taskName").val()).trigger('change');
		},
		close: function() {
			$("#ProjectCategory option:eq(0)").attr("selected", "selected");
			$("#ProjectName option:eq(0)").attr("selected", "selected");
			$("#TaskType option:eq(0)").attr("selected", "selected");
			//$("#taskName option:eq(0)").attr("selected", "selected");
			$("#TaskTimes").attr("value","");
		}
	});
	...
	$("#display-event-form").dialog({
		...
		buttons: {		
			...
			'Edit': function() {
				...
				// $("#alter_projectID").select2("val", projectID);
				 $(this).dialog('close');
			},
			'Delete': function() {
				if(confirm(notice11)){
	...
	//更新一条工数数据
	function initCalendarWithAgendaItems(initDate,TaskTimes,TaskID,TaskType,ProjectName,ProjectCategory,manhourID,projectID,ComponentID,ComponentName){
        if(TaskType == null){
            var TaskType = "-";
        }
    ...
    $.ajax({ 
        type: "post", 
        url: "calendar/listCT.do?currentDate="+currentDate, 
        dataType: "json", 
        success: function (data) {
            $("#deadline").attr("value",data.deadline);
            $("#stopDate").attr("value",data.stopDate);
            $("#monthEndFlag").attr("value",data.monthEndFlag);
            //add by liu
            var totalHour = 0.0;
            var tempDate = "";
            var tempStr =  "";
            ...
            $.each(data.result,function(index,content) {
               ...
               if (tempStr == "" || tempStr == dateStr){
            	   ...
            	   for (k = 0; k < i ;k++){
            		   ...
            		   initCalendarWithAgendaItems(tempDate,tempContent.times,tempContent.taskID,tempContent.task
            				   ,tempContent.projectName,tempContent.category,tempContent.id,tempContent.projectID,tempContent.componentid,tempContent.componentName);
            	   }
            	   i = 0;
            ...
            $.ajax({ 
                type: "post", 
                url: "calendar/listCTProjectInfo.do", 
                dataType: "json", 
                success: function (data) {
                	
                	var val=["3 ->", "3_1 ->", "3_1_2 ->", "3_1_2_1"];
                	var sel=["categoryName", "projectID", "componentID", "taskName"];
                	var sel1=["alter_categoryName", "alter_projectID","alter_componentID", "alter_taskName"];
...
CascadeSelect.prototype = {
  ...
  initialize: function(arrSelects, arrMenu, options) {
    ...
    if(arrSelects.length <= 0 || arrMenu.length <= 0) return;//菜单对象
  ...
  SetOptions: function(options) {
    this.options = {//默认值
        ...
        EmptyText:    ""//空值显示文本(ShowEmpty为true时有效)
    };
  ...
  Set: function(index) {
    ...
    if(index == 0){
        ...
        $("#componentID").select2("val", ""); 
        $("#alter_componentID").select2("val", "");
	    $("#taskName").select2("val", "");
        $("#alter_taskName").select2("val", "");
    }
    //清空默认值
...
//SELECT
	...
	<div id="tabs">
		...
		<input type="hidden" id="monthEndFlag" name="monthEndFlag" value=""></input>
		<input type="hidden" id="stopDate" name="monthEndFlag" value=""></input>
		<div id="tabs-2">
	...
	padding: 0.3em;
			...
			<div id="add-event-form" title="新增工数">
				<form>
					...
					<input type=hidden id="date" name="date" value=clickDate>
					...
					<fieldset>
						...
						<table style="width: 100%; padding: 5px;">
							<tr>
								...
								<td style="width: 80%;">
									<select name="categoryName"	id="categoryName" class="text ui-widget-content ui-corner-all"
							...
							<tr>
								<td style="width: 20%;"><label id="customDemo_string_componentID" >コンポーネント</label></td>
								<td style="width: 80%;">
									<select name="componentID" id="componentID"></select>
								<td>
							</tr>
							<!-- <tr>
								<td style="width: 20%;"><label id="customDemo_string_taskName" >作业类型</label></td>
								<td style="width: 80%;">
									<select name="taskName1" id="taskName1" class="text ui-widget-content ui-corner-all"
										style="margin-bottom: 12px; width: 95%; padding: .4em;">
									</select>
								<td>
							</tr> -->
							<tr>
								<td style="width: 20%;"><label id="customDemo_string_taskName" >作业类型</label></td>
								<td style="width: 80%;">
									<select name="taskName" id="taskName"></select>
								<td>
							</tr>
							<tr>
								<td style="width: 20%;"><label id="customDemo_string_TaskTimes" >工作时间(小时)</label></td>
								<td style="width: 80%;">
									<input type="text" name="times"	id="TaskTimes" class="text ui-widget-content ui-corner-all"
									style="margin-bottom: 12px; width: 95%; padding: .4em;" />
								<td>
							</tr>
			...
			<div id="alter-event-form" title="修改工数">
				<form>
					<b><p name="AlterEventCurrentDay" id="AlterEventCurrentDay"	class="validateTips" align="center"
							style="color: #e64545; font-size: 14px;">
							All form fields are	required.</p>
					</b> 
					<input type="hidden" id="alter_date" name="alter_date" value=clickDate>
					<fieldset>
						<table style="width: 100%; padding: 5px;">
							<tr>
								<td style="width: 20%;"><label id="customDemo_string_altercategoryName" >项目类别</label></td>
								<td style="width: 80%;">
									<select name="alter_categoryName" id="alter_categoryName"
							...
							<tr>
								...
								<td style="width: 80%;">
									<select name="alter_projectID" id="alter_projectID"></select>
								<td>
							</tr>
							<tr>
								<td style="width: 20%;"><label id="customDemo_string_alterComponentID" ></label></td>
								<td style="width: 80%;">
									<select name="alterComponentID" id="alter_componentID"></select>
								<td>
							</tr>
							<tr>
								<td style="width: 20%;"><label id="customDemo_string_altertaskName" >作业类型</label></td>
								<td style="width: 80%;">
									<select name="alter_taskName" id="alter_taskName"></select>
								<td>
							</tr>
							...
							<tr>
								...
								<td style="width: 80%;">
									<input type="text" name="alter_times" id="alter_TaskTimes"
									class="text ui-widget-content ui-corner-all"
									style="margin-bottom: 12px; width: 95%; padding: .4em;" />
								<td>
							</tr>
